...
Путь вперёд (не сделано в этой сессии — требует глубже, чем перебор
файлов VBIOS):
1. Собрать/найти debug-сборку OVMF (на ноде только release-прошивка,
   ...
   реальный DXE-лог через debug-консоль — единственный способ увидеть,
   на каком именно вызове/протоколе конкретно виснет исполнение, вместо
   догадок по косвенным признакам.
2. Поискать известные OVMF/QEMU quirks именно для "GOP passthrough вешает
   DXE" — начать с `ConnectController`/автоконнекта дисплея, quirks вида
   `-global vfio-pci.x-igd-...` (обычно для intel iGPU, но стоит
   проверить аналоги для дискретных карт), и версии QEMU/OVMF (у ноды
   `pve-edk2-firmware` от 2025-11-13 — возможно, известный баг, уже
   исправленный в более новой прошивке).
3. Только если 1-2 не дадут ответа — возвращаться к перебору VBIOS-файлов
   (не тестировался четвёртый кандидат, Asus GTX 950, оставлен на ноде
   как `/usr/share/kvm/gtx950-asus.rom`), хотя три из трёх уже
   одинаковых результатов делают это менее вероятным следующим шагом.
4. Если стабилизируется — прогнать OCLP `--patch_sys_vol` заново (теперь
