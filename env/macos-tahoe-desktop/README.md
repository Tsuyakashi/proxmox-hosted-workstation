...
1. Собрать/найти debug-сборку OVMF и получить реальный DXE-лог — 
   единственный способ увидеть точку зависания напрямую, а не по
   косвенным признакам. Не пробовалось: сборка edk2 из исходников с
   `DEBUG_ON_SERIAL_PORT` вручную (нода не даёт готовой debug-прошивки).
2. Учитывая находку про QEMU's `pci-quirks.c` — можно попробовать
   собрать QEMU с патчем, специально обрабатывающим этот чип похожим на
   BCM57810 образом (форсировать пропуск проблемного ожидания) — требует
   пересборки QEMU, не тривиально.
