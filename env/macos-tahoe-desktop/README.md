...
Путь вперёд (не сделано в этой сессии — реально требует глубже, чем
перебор файлов VBIOS и известных community quirks, оба пути исчерпаны):
1. Собрать/найти debug-сборку OVMF (на ноде только release-прошивка,
   `/usr/share/pve-edk2-firmware/` — ни одной `.debug.fd`) и получить
   реальный DXE-лог через debug-консоль — единственный оставшийся способ
   увидеть, на каком именно вызове/протоколе конкретно виснет исполнение,
   вместо догадок по косвенным признакам (сейчас известно только: диск не
   читается вообще, значит зависание строго до передачи управления
   загрузчику/ОС).
2. Проверить версию `pve-edk2-firmware` на предмет известных багов —
   нода на сборке от 2025-11-13, возможно есть более новая с фиксом
   именно для GOP+passthrough сценария.
3. Четвёртый кандидат VBIOS (Asus GTX 950) не тестировался — оставлен на
   ноде как `/usr/share/kvm/gtx950-asus.rom`, но после того как 3 из 3
   разных источников VBIOS **и** известный community-фикс (`kernel-irqchip`)
   не сработали одинаково, маловероятно, что четвёртый файл даст другой
   результат — не приоритетный следующий шаг.
4. Если когда-нибудь стабилизируется — прогнать OCLP `--patch_sys_vol`
   заново (теперь карта реально видна железу) для настоящего non-Metal
   patchset (не только Legacy USB), и уже тогда оценивать non-Metal-
   ускорение как таковое.

Механизм (`gpu_rom_file` → `mod/vm`'s `rom_file` → живой `qm showcmd`)
подтверждён рабочим и переиспользуемым — когда найдётся способ не
подвешивать OVMF, включение сведётся к одной команде (`terraform apply
-var gpu_primary=true -var gpu_rom_file=<файл>`) без дальнейших правок
кода. Все протестированные файлы оставлены на ноде: `gtx950.rom`
(референсный NVIDIA), `gtx950-combined.rom` (склейка), `gtx950-msi.rom`,
`gtx950-asus.rom` (не тестировался).
