...
`0`) ещё до того, как гость успевает подняться по сети. Один раз загрузка
не завершилась за 5+ минут при устойчиво высокой загрузке CPU процесса
QEMU (похоже на зависание, не точно — в этой сессии уже бывали
легитимные многоминутные паузы при полностью рабочей загрузке, например
Setup Assistant зависал на 20+ минут и в итоге прошёл сам). Не успел
дождаться повторной попытки и доверифицировать через SSH/`ioreg`/вебку до
необходимости прерваться — **следующий шаг сессии именно это**.

Путь вперёд:
1. Дождаться/повторить холодный старт, дать побольше времени (10+ минут),
   проверить `ioreg`/`system_profiler` по SSH и физический вывод через
   вебку, направленную на мониторы (уже физически настроена пользователем
   на bare-pve).
2. Если гонка с PCIe-сбросами при старте подтвердится как причина
   нестабильности — посмотреть в сторону OpenCore-квирка
   `ProvideConsoleGop`/`ReloadOptionRoms` (сейчас `ReloadOptionRoms=false`
   в собранном `config.plist` — потенциально заставляет прошивку
   перевыполнять ROM при каждом сбросе шины, а не один раз при первом
   DXE-проходе) — требует пересборки OpenCore ISO.
...
`gpu_primary` и `gpu_rom_file` **оставлены в применённом состоянии**
(`true` / `gtx950.rom`) — VM 103 сейчас настроена именно на путь,
который реально показал рабочий результат хотя бы часть времени, а не
откачена обратно на заведомо нерабочий (без карты вообще) std-VGA. Если
после перезапуска сессии `ioreg`/вебка не подтвердят стабильную видимость
— следующий шаг — первый в списке выше, а не пересмотр самого подхода: VBIOS
подход подтверждён работающим по существу, вопрос только в стабильности
момента исполнения ROM относительно сбросов PCIe-линка на старте.
