...
## Состояние на утро 2026-09-16

**bare-pve, VM 104 — macOS Tahoe 26 установлена с нуля и работает.**
Установка прошла полностью (скачивание ~19 ГБ, все фазы, перезагрузки),
Setup Assistant пройден, учётная запись создана: полное имя `Daniil`,
короткое имя **`tsu`**, пароль **`meow`** (те же реквизиты, что и у гостя
на pve-rog — чтобы не плодить разные). Рабочий стол поднимается, Finder,
Dock, виджеты на месте; проброшенная физическая USB-клавиатура (SONiX)
определяется в госте.

Один нюанс установки, подтверждающий старую находку: Setup Assistant
снова **завис на шаге "Update Mac Automatically"**. Лечится ровно как
записано в `env/macos-tahoe-desktop/README.md` — `qm stop` + `qm start`,
прогресс мастера переживает перезагрузку и система выходит сразу на экран
логина.

**Вывод на физический монитор для VM 104 пока не получен.** Связка,
которая сработала на VM 103 (`x-vga=1` + `romfile=gtx950.rom`), здесь не
загружается: чтение с диска замирает на ~84 МБ (до OpenCore дело не
доходит), клавиши не помогают. Отдельно выяснилось, что при `x-vga=1`
Proxmox запускает QEMU с `-vga none` и **без эмулированной клавиатуры**
вообще (`sendkey` отправлять некуда) — для таких тестов нужен явный
`-device usb-kbd` в `args`; добавление его ситуацию не изменило, так что
дело не в вводе, а в самой инициализации GOP на этой конфигурации.
VM 104 возвращена в проверенно рабочее состояние (`x-vga=0`), доступна
через консоль Proxmox.

**pve-rog, VM 102 — Kepler-карта проброшена, OCLP готов к запуску.**
IOMMU включён, `GTX 770M` и её HDMI-аудио привязаны к `vfio-pci`, карта
видна гостю как настоящий GPU (`PCIe, Slot-1, x16, NVIDIA 0x10de/0x11e0`,
`Kernel Extension Info: No Kext Loaded`). OCLP 2.5.0 установлен, гейт
версии ОС снят. Осталось только то, что требует решения владельца — см.
раздел ниже.
